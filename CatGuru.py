...
class CatGuru:
    ...
    def __init__(self):
        ...
        )  # Use .place for full window coverage.
        ...
        self.avatar_label.pack(
            padx=20, pady=20
        )  # Pack the avatar label into the window.

        # --- Options Panel (Buttons) Setup ---
        # Frame to hold the action buttons at the bottom of the window.
        self.options_frame = tk.Frame(
            self.window, bg=self.background_color
        )  # Set background to current theme.
        self.options_frame.pack(
            side=tk.BOTTOM, pady=30
        )  # Pack to the bottom, with vertical padding.

        # Label to display the fetched cat wisdom/fact.
        self.wisdom_label = tk.Label(self.window, text="", font=("Arial", 12))
        self.wisdom_label.config(
            bg=self.background_color
        )  # Ensure wisdom label background matches theme.
        self.wisdom_label.pack(
            side=tk.BOTTOM, fill=tk.X, padx=10, pady=10
        )  # Pack below avatar, allow horizontal fill.

        # Inner frame to hold buttons, allowing for centering of the button group.
        self.button_container_frame = tk.Frame(
            self.options_frame, bg=self.background_color
        )  # Match parent background.
        self.button_container_frame.pack(
            side=tk.TOP
        )  # Pack within options_frame, enabling centering of buttons.

        # --- Buttons ---
        # "Ask the Cat Guru" button to fetch and display a new cat fact.
        self.wisdom_button = customtkinter.CTkButton(
            master=self.button_container_frame,  # Master is the button container.
        ...
        self.wisdom_button.pack(
            side=tk.LEFT, padx=5
        )  # Pack buttons side-by-side with horizontal padding.

        # "Change Avatar" button to cycle through available cat avatars.
        self.avatar_button = customtkinter.CTkButton(
            master=self.button_container_frame,
            text="Change Avatar",
        ...
        self.background_button = customtkinter.CTkButton(
            master=self.button_container_frame,
        ...
        self.background_button_tooltip = (
            "Bored of your old background? Change it to a color that's meow-gical!"
        )
    ...
    def change_avatar(self):
        ...
        """
        self.avatar_index = (self.avatar_index + 1) % len(
            AVATARS
    ...
    def load_avatar_image(self, filename):
        ...
        if filename not in self.avatar_cache:  # Check if the image is already cached.
            ...
            self.avatar_cache[filename] = ImageTk.PhotoImage(
                image
            )  # Store PhotoImage in cache.
        return self.avatar_cache[
            filename
        ]  # Return the cached (or newly loaded) PhotoImage.
